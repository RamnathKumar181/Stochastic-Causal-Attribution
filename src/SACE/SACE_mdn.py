...
def compute_ace_mdn(X, model, num_c=5, num_alpha=1000,
                    title="test", mode="middle"):
    final = []
    ...
    for t in range(0, num_c):
        ...
        for x in np.linspace(0, 1, num_alpha):
            ...
            input_torchvar = autograd.Variable(torch.FloatTensor(inp),
                                               requires_grad=True)
            _, prediction_mu, prediction_sigma = model(input_torchvar)
            if mode == "middle":
                output = prediction_mu
            elif mode == "upper":
                output = prediction_mu+prediction_sigma
            else:
                output = prediction_mu-prediction_sigma
            o1 = output.data.cpu()
            val = o1.numpy()[0]
...
def plot_sace_mdn(x, net, filename, title, num_c=5, extend_plot=False,
                  feature_names=None):
    ...
    col = {0: "b", 1: "g", 2: "r", 3: "c", 4: "y", 5: "m", 6: '#A52A2A',
           ...
           11: "#DDA0DD", 12: "#8B4513"}
    causal_effect_middle = compute_ace_mdn(X=x.numpy(),
                                           model=net,
                                           num_c=num_c,
                                           mode="middle")
    causal_effect_lower = compute_ace_mdn(X=x.numpy(),
                                          model=net,
                                          num_c=num_c,
                                          mode="lower")
    causal_effect_upper = compute_ace_mdn(X=x.numpy(),
                                          model=net,
                                          num_c=num_c,
                                          mode="upper")
...
def plot_sace_mdn_saliency_map(x, net, filename, title, num_c=5,
                               feature_names=None):
    causal_effect_middle = compute_ace_mdn(X=x.numpy(),
                                           model=net,
                                           num_c=num_c,
